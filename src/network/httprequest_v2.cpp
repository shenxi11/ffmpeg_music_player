...
QString normalizePlaylistOwnershipToken(QString value) {
    ...
    if (value == QStringLiteral("owned") || value == QStringLiteral("owner") ||
        value == QStringLiteral("created") || value == QStringLiteral("creator") ||
        value == QStringLiteral("mine") || value == QStringLiteral("self")) {
        return QStringLiteral("owned");
    }
    if (value == QStringLiteral("subscribed") || value == QStringLiteral("subscription") ||
        value == QStringLiteral("collected") || value == QStringLiteral("favorite") ||
        value == QStringLiteral("favorited") || value == QStringLiteral("collected_playlist")) {
        return QStringLiteral("subscribed");
    }
    return QString();
}

QString normalizePlaylistOwnershipFlag(const QJsonValue& value, const QString& trueLabel,
...
QString normalizePlaylistOwnership(const QJsonObject& obj) {
    static const QStringList stringKeys = {
        QStringLiteral("ownership"), QStringLiteral("playlist_type"), QStringLiteral("source_type"),
    ...
    static const QStringList ownedKeys = {QStringLiteral("is_owned"), QStringLiteral("owned"),
                                          QStringLiteral("is_owner")};
    for (const QString& key : ownedKeys) {
        if (obj.contains(key)) {
            const QString normalized = normalizePlaylistOwnershipFlag(
                obj.value(key), QStringLiteral("owned"), QStringLiteral("subscribed"));
            if (!normalized.isEmpty()) {
                return normalized;
            }
        }
    ...
    static const QStringList subscribedKeys = {QStringLiteral("is_subscribed"),
                                               QStringLiteral("subscribed"),
                                               QStringLiteral("is_favorited")};
    for (const QString& key : subscribedKeys) {
        if (obj.contains(key)) {
            const QString normalized = normalizePlaylistOwnershipFlag(
                obj.value(key), QStringLiteral("subscribed"), QStringLiteral("owned"));
            if (!normalized.isEmpty()) {
                return normalized;
            }
        }
...
QString readDurationFromObject(const QJsonObject& obj) {
    static const QStringList keys = {"duration", "duration_sec", "durationSec", "duration_ms",
                                     "durationMs"};
...
QString readArtistFromObject(const QJsonObject& obj) {
    ...
    static const QStringList directKeys = {"artist", "singer", "author", "artist_name", "name"};
    ...
    static const QStringList arrayKeys = {"artists", "ar"};
    ...
    static const QStringList nestedKeys = {"music", "song", "track", "file"};
...
QJsonObject unwrapDataObject(const QJsonObject& rootObj) {
    if (rootObj.contains(QStringLiteral("data")) &&
        rootObj.value(QStringLiteral("data")).isObject()) {
...
bool parseSuccessFlag(const QJsonObject& obj, bool defaultValue) {
    ...
    if (obj.contains(QStringLiteral("success_bool")) &&
        obj.value(QStringLiteral("success_bool")).isBool()) {
...
QString rewriteServiceUrlToBase(const QString& rawUrl, const QString& baseUrl) {
    ...
    if (lowerTrimmed == QStringLiteral("null") || lowerTrimmed == QStringLiteral("undefined") ||
        lowerTrimmed == QStringLiteral("none") || lowerTrimmed == QStringLiteral("(null)")) {
        return QString();
    }
    ...
    if (normalizedPath.size() >= 3 && normalizedPath[1] == QLatin1Char(':') &&
    ...
    if (trimmed.startsWith("http://", Qt::CaseInsensitive) ||
        ...
        if (lowerPath.startsWith(QStringLiteral("/uploads/uploads/"))) {
            fixedPath = QStringLiteral("/uploads/") +
                        fixedPath.mid(QStringLiteral("/uploads/uploads/").size());
    ...
    if (fixedLower.startsWith(QStringLiteral("uploads/uploads/"))) {
        fixedRelative = QStringLiteral("uploads/") +
                        fixedRelative.mid(QStringLiteral("uploads/uploads/").size());
    ...
    if (fixedRelativeLower.startsWith("uploads/") || fixedRelativeLower.startsWith("video/") ||
        fixedRelativeLower.startsWith("hls/") || fixedRelativeLower.startsWith("files/") ||
        fixedRelativeLower.startsWith("download") || fixedRelativeLower.startsWith("lrc")) {
...
QString normalizeOnlineMediaPath(const QString& rawPath, const QString& baseUrl) {
    ...
    if (lower.startsWith("uploads/") || lower.startsWith("video/") || lower.startsWith("hls/")) {
    ...
    m_networkService.postJson(
        ...
        [this, account, password](const Network::NetworkResponse& response) {
            if (response.isSuccess()) {
                ...
                qDebug() << "[HttpRequestV2] Login response received in" << response.elapsedMs
                         << "ms";
                ...
                if (loginSuccess) {
                    ...
                    avatarUrl =
                        rewriteServiceUrlToBase(result.value("avatar_url").toString(), m_baseUrl);

                    const QString onlineToken =
                        result.value("online_session_token").toString().trimmed();
                    const int heartbeatSec =
                        result.value("online_heartbeat_interval_sec").toInt(30);
...
void HttpRequestV2::registerUser(const QString& account, const QString& password,
    ...
    m_networkService.postJson(
        "users/register", json, options, [this](const Network::NetworkResponse& response) {
            if (response.isSuccess()) {
                ...
                qWarning() << "[HttpRequestV2] Registration error:" << response.statusCode
                           << message;
...
void HttpRequestV2::resetPassword(const QString& account, const QString& newPassword) {
    ...
    m_networkService.postJson(
        "users/reset_password", json, options, [this](const Network::NetworkResponse& response) {
            if (response.isSuccess()) {
                qDebug() << "[HttpRequestV2] Reset password successful";
                emit signalResetPasswordResult(true, QString::fromUtf8("瀵嗙爜閲嶇疆鎴愬姛"));
            } else {
                const QString message = extractErrorMessage(response, response.errorString);
                qWarning() << "[HttpRequestV2] Reset password error:" << response.statusCode
                           << message;
...
void HttpRequestV2::getAllFiles(bool useCache) {
    ...
    m_networkService.get("files", options, [this](const Network::NetworkResponse& response) {
        if (response.isSuccess()) {
            ...
            qDebug() << "[HttpRequestV2] Received" << filesArray.size() << "files"
                     << (response.isFromCache ? "(from cache)" : "(from server)") << "in"
            ...
            for (const QJsonValue& value : filesArray) {
                if (value.isObject()) {
                    ...
                    QString coverUrl =
                        rewriteServiceUrlToBase(fileObj["cover_art_url"].toString(), m_baseUrl);
...
void HttpRequestV2::getLyrics(const QString& url) {
    ...
    m_networkService.get(lrcUrl, options, [this](const Network::NetworkResponse& response) {
        if (response.isSuccess()) {
            QJsonArray filesArray = Network::NetworkService::parseJsonArray(response);
            QStringList lines;
...
void HttpRequestV2::downloadFile(const QString& filename, const QString& downloadFolder,
    ...
    DownloadManager::instance().addDownload(downloadUrl, filename, downloadFolder, postData,
                                            coverUrl);
    ...
    if (downloadLyrics) {
        ...
        DownloadManager::instance().addDownload(downloadUrl, lrcFilename, downloadFolder,
                                                lrcPostData);
    }
}

void HttpRequestV2::addFavorite(const QString& userAccount, const QString& path,
    ...
    QString url =
        "user/favorites/add?user_account=" + QString(QUrl::toPercentEncoding(userAccount));
    ...
    m_networkService.postJson(
        url, json, options, [this, title, userAccount](const Network::NetworkResponse& response) {
            if (response.isSuccess()) {
                QJsonObject result = Network::NetworkService::parseJsonObject(response);
                ...
                if (success) {
                    m_networkService.invalidateCache(
                        QString("user/favorites?user_account=%1").arg(userAccount));
                }
                qDebug() << "[HttpRequestV2] Add favorite" << (success ? "success" : "failed")
                         << title;
                ...
                if (alreadyExists) {
                    qDebug() << "[HttpRequestV2] Favorite already exists, treat as success:"
                             << title;
                    m_networkService.invalidateCache(
                        QString("user/favorites?user_account=%1").arg(userAccount));
                ...
                qWarning() << "[HttpRequestV2] Add favorite error:" << response.errorString
                           << "status:" << response.statusCode << "body:" << bodyText;
                emit signalAddFavoriteResult(false);
            }
        });
}
...
void HttpRequestV2::getFavorites(const QString& userAccount, bool useCache) {
    ...
    m_networkService.get(url, options, [this](const Network::NetworkResponse& response) {
        if (response.isSuccess()) {
            // 服务端直接返回 JSON 数组
            ...
            for (const QJsonValue& value : favoritesArray) {
                ...
                if (!isLocal) {
                    ...
                    if (normalized != path) {
                        qDebug() << "[HttpRequestV2] Favorite path normalized:" << path << "->"
                                 << normalized;
                    }
                    path = normalized;
                }
                ...
                QString coverArtUrl =
                    rewriteServiceUrlToBase(favObj["cover_art_url"].toString(), m_baseUrl);
                if (isLocal && coverArtUrl.trimmed().isEmpty()) {
                    ...
                    if (!key.isEmpty()) {
                        ...
                        if (!localCover.trimmed().isEmpty()) {
                            ...
                            qDebug() << "[HttpRequestV2] Filled local favorite cover from cache:"
                                     << path;
                        }
                    }
                }
                ...
                rememberCoverForSongMeta(favObj["title"].toString(), readArtistFromObject(favObj),
                                         coverArtUrl);
...
void HttpRequestV2::addPlayHistory(const QString& userAccount, const QString& path,
                                   const QString& title, const QString& artist,
                                   const QString& album, const QString& duration, bool isLocal) {
    if (userAccount.trimmed().isEmpty()) {
    ...
    m_networkService.postJson(
        url, json, options, [this, title](const Network::NetworkResponse& response) {
            if (response.isSuccess()) {
                QJsonObject result = Network::NetworkService::parseJsonObject(response);
                bool success = result.value("success").toBool();
                emit signalAddHistoryResult(success);
            } else {
                qDebug() << "[HttpRequestV2] Add history failed (not critical):"
...
void HttpRequestV2::getPlayHistory(const QString& userAccount, int limit, bool useCache) {
    ...
    qDebug() << "[HttpRequestV2] Getting play history for:" << userAccount << "limit:" << limit
             << "useCache:" << useCache;
        ...
        if (response.isSuccess()) {
            ...
            for (const QJsonValue& value : historyArray) {
                ...
                if (!isLocal) {
                    const QString normalized = normalizeOnlineMediaPath(path, m_baseUrl);
                    if (normalized != path) {
                        qDebug() << "[HttpRequestV2] History path normalized:" << path << "->"
                                 << normalized;
                    }
                    path = normalized;
                }
                ...
                QString historyCover =
                    rewriteServiceUrlToBase(histObj["cover_art_url"].toString(), m_baseUrl);
                ...
                rememberCoverForSongMeta(histObj["title"].toString(), readArtistFromObject(histObj),
                                         historyCover);
...
void HttpRequestV2::getAudioRecommendations(const QString& userId, const QString& scene, int limit,
                                            bool excludePlayed, const QString& cursor) {
    ...
    query.addQueryItem(QStringLiteral("scene"),
                       scene.trimmed().isEmpty() ? QStringLiteral("home") : scene.trimmed());
    query.addQueryItem(QStringLiteral("limit"), QString::number(safeLimit));
    query.addQueryItem(QStringLiteral("exclude_played"),
                       excludePlayed ? QStringLiteral("true") : QStringLiteral("false"));
    ...
    m_networkService.get(url, options, [this](const Network::NetworkResponse& response) {
        ...
        QVariantMap meta;
        meta.insert(QStringLiteral("request_id"),
                    dataObj.value(QStringLiteral("request_id")).toString());
        ...
        meta.insert(QStringLiteral("model_version"),
                    dataObj.value(QStringLiteral("model_version")).toString());
        meta.insert(QStringLiteral("next_cursor"),
                    dataObj.value(QStringLiteral("next_cursor")).toString());
        ...
        for (const QJsonValue& value : itemArray) {
            ...
            QVariantMap item;
            item.insert(QStringLiteral("song_id"),
                        itemObj.value(QStringLiteral("song_id")).toString());
            ...
            item.insert(QStringLiteral("lrc_url"),
                        rewriteServiceUrlToBase(itemObj.value(QStringLiteral("lrc_url")).toString(),
                                                m_baseUrl));
            item.insert(QStringLiteral("score"), itemObj.value(QStringLiteral("score")).toDouble());
            item.insert(QStringLiteral("reason"),
                        itemObj.value(QStringLiteral("reason")).toString());
            ...
            item.insert(QStringLiteral("model_version"),
                        meta.value(QStringLiteral("model_version")));
            ...
            rememberCoverForSongMeta(itemObj.value(QStringLiteral("title")).toString(),
                                     readArtistFromObject(itemObj), coverArtUrl);
            items.append(item);
        }
...
void HttpRequestV2::getSimilarRecommendations(const QString& songId, int limit) {
    ...
    qDebug() << "[HttpRequestV2] Getting similar recommendations for songId:" << trimmedSongId
             << "limit:" << safeLimit;

    m_networkService.get(
        url, options, [this, trimmedSongId](const Network::NetworkResponse& response) {
            if (!response.isSuccess()) {
            ...
            meta.insert(QStringLiteral("request_id"),
                        dataObj.value(QStringLiteral("request_id")).toString());
            meta.insert(QStringLiteral("user_id"),
                        dataObj.value(QStringLiteral("user_id")).toString());
            meta.insert(QStringLiteral("scene"), dataObj.value(QStringLiteral("scene")).toString());
            meta.insert(QStringLiteral("model_version"),
                        dataObj.value(QStringLiteral("model_version")).toString());
            meta.insert(QStringLiteral("next_cursor"),
            ...
            for (const QJsonValue& value : itemArray) {
                ...
                item.insert(QStringLiteral("lrc_url"),
                            rewriteServiceUrlToBase(
                                itemObj.value(QStringLiteral("lrc_url")).toString(), m_baseUrl));
                item.insert(QStringLiteral("score"),
                            itemObj.value(QStringLiteral("score")).toDouble());
                item.insert(QStringLiteral("reason"),
                            itemObj.value(QStringLiteral("reason")).toString());
                item.insert(QStringLiteral("source"),
                            itemObj.value(QStringLiteral("source")).toString());
...
void HttpRequestV2::postRecommendationFeedback(const QString& userId, const QString& songId,
                                               ...
                                               const QString& modelVersion, qint64 playMs,
                                               qint64 durationMs) {
    const QString trimmedUser = userId.trimmed();
    const QString trimmedSongId = songId.trimmed();
    ...
    Network::RequestOptions options =
        Network::RequestOptions::withPriority(Network::RequestPriority::Low);
    options.maxRetries = 0;
    options.useCache = false;

    m_networkService.postJson(
        QStringLiteral("recommendations/feedback"), json, options,
        [this, trimmedEventType, trimmedSongId](const Network::NetworkResponse& response) {
            ...
            if (response.isSuccess()) {
                ...
                qWarning() << "[HttpRequestV2] Recommendation feedback error:"
                           << response.errorString << "status:" << response.statusCode;
            }
...
void HttpRequestV2::getVideoList() {
    ...
    m_networkService.get("videos", options, [this](const Network::NetworkResponse& response) {
        if (response.isSuccess()) {
...
void HttpRequestV2::getVideoStreamUrl(const QString& videoPath) {
    ...
    m_networkService.postJson(
        "video/stream", json, {}, [this](const Network::NetworkResponse& response) {
            if (response.isSuccess()) {
                QJsonObject result = Network::NetworkService::parseJsonObject(response);
...
void HttpRequestV2::searchArtist(const QString& artist) {
    ...
    m_networkService.postJson(
        "artist/search", json, options, [this, artist](const Network::NetworkResponse& response) {
            if (response.isSuccess()) {
                QJsonObject result = Network::NetworkService::parseJsonObject(response);
...
void HttpRequestV2::getMusicByArtist(const QString& artist) {
    ...
    m_networkService.postJson(
        "music/artist", json, options, [this, artist](const Network::NetworkResponse& response) {
            if (response.isSuccess()) {
...
void HttpRequestV2::getMusicData(const QString& fileName) {
    ...
    m_networkService.post(url, postData, options, [this](const NetworkResponse& response) {
        if (response.isSuccess()) {
            ...
            if (jsonDoc.isObject()) {
                ...
                if (obj.contains("stream_url")) {
                    QString streamUrl =
                        rewriteServiceUrlToBase(obj["stream_url"].toString(), m_baseUrl);
    ...
    m_networkService.post(url, postData, Network::RequestOptions(),
                          [musicPath](const Network::NetworkResponse& response) {
                              if (response.isSuccess()) {
                                  ...
                                  qWarning()
                                      << "[HttpRequestV2] Add music error:" << response.errorString;
                              }
                          });
}
...
void HttpRequestV2::getMusic(const QString& keyword) {
    ...
    m_networkService.postJson(
        "music/search", json, Network::RequestOptions::withCache(),
...
void HttpRequestV2::removeFavorite(const QString& userAccount, const QStringList& paths) {
    ...
    m_networkService.postJson(
        "user/favorites/remove", json, Network::RequestOptions(),
...
void HttpRequestV2::removePlayHistory(const QString& userAccount, const QStringList& paths) {
    ...
    QString url =
        "user/history/delete?user_account=" + QString(QUrl::toPercentEncoding(userAccount));

    qDebug() << "[HttpRequestV2] Deleting play history for user:" << userAccount
             << "paths count:" << paths.size();

    m_networkService.postJson(
        url, json, Network::RequestOptions(),
        [this, userAccount](const Network::NetworkResponse& response) {
            bool success = false;
            int deletedCount = 0;
            ...
            if (response.isSuccess()) {
                ...
                qDebug() << "[HttpRequestV2] Delete history success:" << success
                         << "deleted count:" << deletedCount;
                if (success) {
                    m_networkService.invalidateCache(
                        QString("user/history?user_account=%1&limit=50").arg(userAccount));
                }
            } else {
...
void HttpRequestV2::getPlaylists(const QString& userAccount, int page, int pageSize,
                                 bool useCache) {
    ...
    m_networkService.get(
        url, options, [this, safePage, safePageSize](const Network::NetworkResponse& response) {
            if (!response.isSuccess()) {
            ...
            const int pageValue = data.value(QStringLiteral("page"))
                                      .toInt(root.value(QStringLiteral("page")).toInt(safePage));
            const int pageSizeValue =
                data.value(QStringLiteral("page_size"))
                    .toInt(root.value(QStringLiteral("page_size")).toInt(safePageSize));
            const int totalValue =
                data.value(QStringLiteral("total"))
                    .toInt(root.value(QStringLiteral("total")).toInt(itemsArray.size()));
            ...
            for (const QJsonValue& value : itemsArray) {
                ...
                const int totalDurationSec =
                    obj.value(QStringLiteral("total_duration_sec")).toInt();

                playlist.insert(QStringLiteral("id"),
                                obj.value(QStringLiteral("id")).toVariant().toLongLong());
                playlist.insert(QStringLiteral("name"),
                                obj.value(QStringLiteral("name")).toString());
                playlist.insert(QStringLiteral("description"),
                ...
                playlist.insert(QStringLiteral("track_count"),
                                obj.value(QStringLiteral("track_count")).toInt());
                playlist.insert(QStringLiteral("total_duration_sec"), totalDurationSec);
                playlist.insert(QStringLiteral("total_duration"),
                                formatDurationFromSeconds(totalDurationSec));
                playlist.insert(QStringLiteral("created_at"),
                                obj.value(QStringLiteral("created_at")).toString());
                playlist.insert(QStringLiteral("updated_at"),
                                obj.value(QStringLiteral("updated_at")).toString());
...
void HttpRequestV2::createPlaylist(const QString& userAccount, const QString& name,
    ...
    Network::RequestOptions options =
        Network::RequestOptions::withPriority(Network::RequestPriority::High);
    options.maxRetries = 0;
    options.useCache = false;

    m_networkService.postJson(
        url, json, options, [this, trimmedUser](const Network::NetworkResponse& response) {
            bool success = false;
            qint64 playlistId = 0;
            ...
            if (response.isSuccess()) {
                ...
                message = parseMessageText(
                    data, parseMessageText(root, success ? QStringLiteral("创建成功")
                                                         : QStringLiteral("创建失败")));
                if (success) {
                    m_networkService.invalidateCache(
                        QString("user/playlists?user_account=%1&page=1&page_size=20")
                            .arg(trimmedUser));
                    m_networkService.invalidateCache(
    ...
    m_networkService.postJson(
        ...
        [this, playlistId, trimmedUser](const Network::NetworkResponse& response) {
            ...
            if (response.isSuccess()) {
                ...
                success = parseSuccessFlag(data, parseSuccessFlag(root, true));
                message = parseMessageText(
                    data, parseMessageText(root, success ? QStringLiteral("删除成功")
                                                         : QStringLiteral("删除失败")));
                if (success) {
                    m_networkService.invalidateCache(
                        QString("user/playlists?user_account=%1&page=1&page_size=20")
                            .arg(trimmedUser));
                    m_networkService.invalidateCache(QString("user/playlists/%1?user_account=%2")
                                                         .arg(playlistId)
                                                         .arg(trimmedUser));
                }
            } else {
...
void HttpRequestV2::updatePlaylist(const QString& userAccount, qint64 playlistId,
    ...
    m_networkService.postJson(
        ...
        [this, playlistId, trimmedUser](const Network::NetworkResponse& response) {
            ...
            if (response.isSuccess()) {
                ...
                success = parseSuccessFlag(data, parseSuccessFlag(root, true));
                message = parseMessageText(
                    data, parseMessageText(root, success ? QStringLiteral("更新成功")
                                                         : QStringLiteral("更新失败")));
                if (success) {
                    m_networkService.invalidateCache(
                        QString("user/playlists?user_account=%1&page=1&page_size=20")
...
void HttpRequestV2::addPlaylistItems(const QString& userAccount, qint64 playlistId,
    ...
    for (const QVariant& value : items) {
        ...
        const QString musicPath =
            map.value(QStringLiteral("music_path")).toString().trimmed().isEmpty()
        ...
        const bool isLocal =
            map.contains(QStringLiteral("is_local"))
        ...
        if (coverArtPath.isEmpty()) {
            ...
            coverArtPath = buildCoverPathForPlaylistPayload(queryCoverForSongMeta(
                titleForLookup, map.value(QStringLiteral("artist")).toString()));
        }
        if (!coverArtPath.isEmpty()) {
    ...
    m_networkService.postJson(
        ...
        [this, playlistId, trimmedUser](const Network::NetworkResponse& response) {
            ...
            if (response.isSuccess()) {
                ...
                const QJsonObject data = unwrapDataObject(root);
                success = parseSuccessFlag(data, parseSuccessFlag(root, true));
                addedCount = data.value(QStringLiteral("added_count"))
                                 .toInt(root.value(QStringLiteral("added_count")).toInt());
                skippedCount = data.value(QStringLiteral("skipped_count"))
                                   .toInt(root.value(QStringLiteral("skipped_count")).toInt());
                message = parseMessageText(
                    data, parseMessageText(root, success ? QStringLiteral("添加成功")
                                                         : QStringLiteral("添加失败")));
                if (success) {
                    m_networkService.invalidateCache(
                        QString("user/playlists?user_account=%1&page=1&page_size=20")
            ...
            emit signalAddPlaylistItemsResult(success, playlistId, addedCount, skippedCount,
                                              message);
        });
}

void HttpRequestV2::removePlaylistItems(const QString& userAccount, qint64 playlistId,
    ...
    m_networkService.postJson(
        ...
        [this, playlistId, trimmedUser](const Network::NetworkResponse& response) {
            ...
            if (response.isSuccess()) {
                ...
                const QJsonObject data = unwrapDataObject(root);
                success = parseSuccessFlag(data, parseSuccessFlag(root, true));
                deletedCount = data.value(QStringLiteral("deleted_count"))
                                   .toInt(root.value(QStringLiteral("deleted_count")).toInt());
                message = parseMessageText(
                    data, parseMessageText(root, success ? QStringLiteral("删除成功")
                                                         : QStringLiteral("删除失败")));
                if (success) {
...
void HttpRequestV2::reorderPlaylistItems(const QString& userAccount, qint64 playlistId,
    ...
    m_networkService.postJson(
        ...
        [this, playlistId, trimmedUser](const Network::NetworkResponse& response) {
            ...
            if (response.isSuccess()) {
                ...
                success = parseSuccessFlag(data, parseSuccessFlag(root, true));
                message = parseMessageText(
                    data, parseMessageText(root, success ? QStringLiteral("排序成功")
                                                         : QStringLiteral("排序失败")));
                if (success) {
                    m_networkService.invalidateCache(QString("user/playlists/%1?user_account=%2")
                                                         .arg(playlistId)
...
void HttpRequestV2::getUserProfile(const QString& account, const QString& sessionToken) {
    ...
    m_networkService.get(path, options, [this](const Network::NetworkResponse& response) {
        ...
        const QString message =
            parseMessageText(rootObj, success ? QString() : QStringLiteral("获取资料失败"));
        emit signalUserProfileResult(
            success, success ? buildUserProfileMap(rootObj, m_baseUrl) : QVariantMap(), message,
            response.statusCode);
    });
}

void HttpRequestV2::updateUsername(const QString& account, const QString& sessionToken,
                                   const QString& username) {
    const QString trimmedAccount = account.trimmed();
    const QString trimmedToken = sessionToken.trimmed();
    ...
    m_networkService.postJson(
        QStringLiteral("users/profile/username"), payload, options,
        [this](const Network::NetworkResponse& response) {
            if (!response.isSuccess()) {
                const QString message = extractErrorMessage(response, response.errorString);
            ...
            const bool success = parseSuccessFlag(rootObj, true);
            const QJsonObject data = unwrapDataObject(rootObj);
            const QString updatedUsername =
                normalizeNullableText(data.value(QStringLiteral("username")).toString());
            const QString message =
                parseMessageText(rootObj, success ? QString() : QStringLiteral("用户名修改失败"));
            emit signalUpdateUsernameResult(success, success ? updatedUsername : QString(), message,
                                            response.statusCode);
        });
}

void HttpRequestV2::uploadAvatar(const QString& account, const QString& sessionToken,
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, manager]() {
        ...
        const bool success =
            reply->error() == QNetworkReply::NoError && parseSuccessFlag(rootObj, true);
        ...
        const QString avatarUrl = rewriteServiceUrlToBase(
            unwrapDataObject(rootObj).value(QStringLiteral("avatar_url")).toString(), m_baseUrl);

        emit signalUploadAvatarResult(success, success ? avatarUrl : QString(), message,
                                      statusCode);
        reply->deleteLater();
        manager->deleteLater();
    });
