    ...
    : QWidget(parent)
{
    ...
    connect(slider, &QSlider::valueChanged, this, &ProcessSlider::onValueChanged);
    connect(slider, &QSlider::sliderReleased, this, &ProcessSlider::signalSliderReleased);
    connect(slider, &QSlider::sliderPressed, this, &ProcessSlider::signalSliderPressed);
    connect(slider, &QSlider::sliderPressed, this, &ProcessSlider::onSliderPressed);
}

void ProcessSlider::onDurationChanged(qint64 value) {
    if (printF) {
...
void ProcessSlider::onIsUpChanged(bool flag){
    QString timeStyle;
    if(flag){
        // 浅色主题下的时间标签样式
        timeStyle = 
...
void ProcessSlider::onBackward(){
    int value = slider->value();
    value = std::max(0, value - 1);
    slider->setValue(value);
    updateTime();
}
void ProcessSlider::onForward(){
    int value = slider->value();
...
void ProcessSlider::onValueChanged(int value){
    updateTime();
}

void ProcessSlider::onSliderPressed()
{
    press_position = slider ? slider->value() : 0;
}

void ProcessSlider::updateTime(){
