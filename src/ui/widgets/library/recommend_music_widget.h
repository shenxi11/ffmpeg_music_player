...
class RecommendMusicWidget : public QQuickWidget {
  ...
  signals:
    void playMusicWithMetadata(const QString& filePath, const QString& musicPath,
                               const QString& title, const QString& artist, const QString& cover,
                               const QString& duration, const QString& songId,
                               const QString& requestId, const QString& modelVersion,
                               const QString& scene);
    void addToFavorite(const QString& filePath, const QString& title, const QString& artist,
                       const QString& duration, bool isLocal);
    void feedbackEvent(const QString& songId, const QString& eventType, int playMs, int durationMs,
                       const QString& scene, const QString& requestId, const QString& modelVersion);
