...
const QStringList kSupportedAvatarExtensions = {
    QStringLiteral("jpg"),
    QStringLiteral("jpeg"),
    QStringLiteral("png"),
    QStringLiteral("webp")
    ...
    : QQuickWidget(parent)
{
    ...
    if (QQuickItem* root = rootObject()) {
        ...
        connect(root, SIGNAL(saveUsernameRequested(QString)),
                this, SLOT(handleSaveUsernameRequested(QString)));
        connect(root, SIGNAL(chooseAvatarRequested()),
                this, SLOT(handleChooseAvatarRequested()));
        connect(root, SIGNAL(favoritesShortcutRequested()),
                this, SIGNAL(favoritesShortcutRequested()));
        connect(root, SIGNAL(historyShortcutRequested()),
                this, SIGNAL(historyShortcutRequested()));
        connect(root, SIGNAL(playlistsShortcutRequested()),
                this, SIGNAL(playlistsShortcutRequested()));
        connect(root, SIGNAL(reloginRequested()),
                this, SIGNAL(reloginRequested()));
    }
}
...
void UserProfileWidget::setStatsData(int favoritesCount, int historyCount, int playlistsCount)
{
    if (QQuickItem* root = rootObject()) {
        QMetaObject::invokeMethod(root,
                                  "setStatsData",
                                  Q_ARG(QVariant, QVariant(favoritesCount)),
...
void UserProfileWidget::handleChooseAvatarRequested()
{
    ...
    if (!kSupportedAvatarExtensions.contains(extension)) {
        setStatusMessage(QStringLiteral("error"), QStringLiteral("头像格式仅支持 jpg/jpeg/png/webp。"));
        return;
    }
...
void UserProfileWidget::invokeRootMethod(const char* method, const QVariant& arg1, const QVariant& arg2)
{
    if (QQuickItem* root = rootObject()) {
