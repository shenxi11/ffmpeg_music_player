...
class LoginWidgetQml : public QObject {
  ...
  public:
    explicit LoginWidgetQml(QObject* parent = nullptr)
        : QObject(parent), m_window(nullptr), m_engine(new QQmlApplicationEngine(this)),
          m_viewModel(new LoginViewModel(this)), isVisible(false) {
        ...
        connect(m_window, SIGNAL(loginRequested(QString, QString)), this,
                SLOT(onLoginRequested(QString, QString)));
        connect(m_window, SIGNAL(quickLoginRequested(QString, QString)), this,
                SLOT(onQuickLoginRequested(QString, QString)));
        connect(m_window, SIGNAL(registerRequested(QString, QString, QString)), this,
                SLOT(onRegisterRequested(QString, QString, QString)));
        connect(m_window, SIGNAL(resetPasswordRequested(QString, QString)), this,
                SLOT(onResetPasswordRequested(QString, QString)));

        connect(m_viewModel, &LoginViewModel::loginSucceeded, this,
                &LoginWidgetQml::onLoginSucceeded);
        connect(m_viewModel, &LoginViewModel::loginFailed, this, &LoginWidgetQml::onLoginFailed);
        connect(m_viewModel, &LoginViewModel::switchToLoginModeRequested, this,
                &LoginWidgetQml::onSwitchToLoginModeRequested);
        connect(m_viewModel, &LoginViewModel::registerCompleted, this,
                &LoginWidgetQml::onRegisterResultMessage);
        connect(m_viewModel, &LoginViewModel::resetPasswordCompleted, this,
                &LoginWidgetQml::onResetPasswordResult);

        connect(m_window, &QQuickWindow::visibleChanged, this,
                &LoginWidgetQml::onWindowVisibleChanged);

        // Keep login page aware of cached account for one-click login.
        setSavedAccount(m_viewModel->cachedAccount(), m_viewModel->cachedPassword(),
    ...
    Q_INVOKABLE void requestLogin(const QString& account, const QString& password,
                                  bool autoLogin = false) {
        performLogin(account, password, autoLogin);
    }

    Q_INVOKABLE void setSavedAccount(const QString& account, const QString& password,
                                     const QString& username) {
        if (!m_window) {
            return;
        }
        QMetaObject::invokeMethod(m_window, "setSavedAccount", Q_ARG(QVariant, QVariant(account)),
  ...
  private slots:
    ...
    void onRegisterRequested(const QString& account, const QString& password,
                             const QString& username) {
    ...
    void onLoginSucceeded(const QString& username, const QString& avatarUrl,
    ...
    void onLoginFailed(const QString& message) {
        if (m_window) {
            QMetaObject::invokeMethod(m_window, "onLoginFailed",
    ...
    void onRegisterResultMessage(bool success, const QString& message) {
        ...
        QMetaObject::invokeMethod(m_window, "onRegisterResult", Q_ARG(QVariant, QVariant(success)),
                                  Q_ARG(QVariant, QVariant(message)));
    }
    ...
    void onResetPasswordResult(bool success, const QString& message) {
        ...
        QMetaObject::invokeMethod(m_window, "onResetPasswordResult",
