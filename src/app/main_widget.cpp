...
QVariantMap MainWidget::agentQueueSnapshot() const
{
    ...
    if (w && w->playbackViewModel()) {
        ...
        snapshot.insert(QStringLiteral("currentIndex"), AudioService::instance().currentIndex());
        snapshot.insert(QStringLiteral("playing"), playbackVm->isPlaying());
        snapshot.insert(QStringLiteral("paused"), playbackVm->isPaused());
        snapshot.insert(QStringLiteral("volume"), AudioService::instance().volume());
        snapshot.insert(QStringLiteral("playMode"),
                        static_cast<int>(AudioService::instance().playMode()));
        return snapshot;
    }
    ...
    snapshot.insert(QStringLiteral("volume"), AudioService::instance().volume());
    snapshot.insert(QStringLiteral("playMode"), static_cast<int>(AudioService::instance().playMode()));
    return snapshot;
}
